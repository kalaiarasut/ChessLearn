...
export const THEME_CATEGORIES = [
  ...
  },
  {
    ...
    themes: [
      ...
      { id: "vukovicMate", label: "Vuković Mate", icon: Crosshair, description: "A rook and knight team up to mate the king, supported by a third piece." },
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 grid grid-cols-8 grid-rows-8">
        {board.map((row, ri) =>
          row.map((code, ci) => (
            <div key={`${ri}-${ci}`} className="flex items-center justify-center p-[5%]">
              {code && (
                <img
...
export default function PuzzlesPage() {
  ...
  const [dailyPuzzle, setDailyPuzzle] = useState<PuzzleEntry | null>(null);
  ...
  useEffect(() => {
    fetch("/api/puzzles?mode=daily&count=1")
      .then((r) => r.json())
      .then((data) => {
        if (data.puzzles?.[0]) setDailyPuzzle(data.puzzles[0]);
      })
      .catch(() => {});
  }, []);

  // Apply opponent's first move for display (puzzle convention)
  const dailyDisplayFen = useMemo(() => {
    if (!dailyPuzzle) return null;
    try {
      const game = new Chess(dailyPuzzle.fen);
      ...
      return dailyPuzzle.fen;
    }
  }, [dailyPuzzle]);

  const playerSide = useMemo(() => {
    if (!dailyPuzzle) return "White";
    try {
      const game = new Chess(dailyPuzzle.fen);
      const firstMove = dailyPuzzle.moves[0];
      if (firstMove) {
      ...
      <main className="flex-1 w-full max-w-[1200px] px-6 py-12 mb-20 md:py-16">
        ...
        <div className="mb-14 md:mb-18">
          ...
          <p className="mt-6 text-[var(--text-muted)] text-xl font-medium max-w-2xl leading-relaxed">
            Sharpen your tactical vision with thousands of puzzles from real games.
            Train at your own pace, race the clock, or challenge your streak.
          </p>
        </div>

        {/* ── Daily Puzzle Hero ── */}
        {dailyPuzzle && dailyDisplayFen && (
          <Link href={`/puzzles/solve?mode=standard&id=${dailyPuzzle.id}`}>
          ...
          <div className="mb-14">
            ...
            <div className="rounded-2xl border border-[var(--border)] bg-gradient-to-br from-[var(--card-from)] to-[var(--card-to)] p-7 shadow-[var(--shadow-card)]">
              <div className="flex flex-wrap gap-4 justify-center md:justify-start">
                ...
                <StatItem
                  label="Best Storm"
                  value={stats.bestStormScore}
                  icon={Zap}
                />
                <StatItem
                  label="Best Streak"
                  value={stats.bestStreakScore}
                  icon={BarChart3}
                />
              </div>
            </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                  {category.themes.map((theme) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`group relative rounded-2xl border p-5 transition-all duration-300 cursor-pointer hover:-translate-y-1 h-full flex flex-col ${
                            isSelected
                              ? "border-[var(--border-hover)] bg-[var(--surface-hover)] shadow-lg"
                              : "border-[var(--border)] bg-[var(--surface-alt)] hover:border-[var(--border-hover)] hover:bg-[var(--surface-hover)] shadow-sm hover:shadow-md"
                          }`}
                        >
          ...
          <div className="rounded-3xl border border-[var(--border)] bg-gradient-to-br from-[var(--surface-alt)] to-[var(--bg)] p-10 md:p-14 shadow-2xl text-center max-w-4xl mx-auto relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="text-[16px] md:text-[18px] text-[var(--text-muted)] leading-relaxed mb-10 max-w-2xl mx-auto font-medium">
                Our platform features millions of tactical puzzles completely in the public domain. 
